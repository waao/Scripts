...
@ScriptManifest(authors = { "MrSneaky" }, keywords = { "Farming", "mrsneaky",
		"herb", "farmer" }, name = "SneakyFarmer", description = "Farms herbs at 3 patches", version = 1.59, website = "http://www.powerbot.org/vb/showthread.php?t=523925", requiresVersion = 244)
public class SneakyFarmer extends Script implements MessageListener,
		PaintListener, MouseListener {

	// Credits go to Unknown500 for the AntiBan skeleton code
	public class AntiBan extends Thread {

		private final SneakyFarmer parent;
		private final Random randomGenerator;

		AntiBan(final SneakyFarmer parent) {
			this.parent = parent;
		...
		public void run() {
			try {
				while (scriptRunning) {
					if (!parent.isPaused() && parent.game.isLoggedIn()
							&& status != "Logging back in") {
						final int rand = randomGenerator.nextInt(30);
						switch (rand) {
						case 1:
	...
	public class HerbFarmerGUI extends JFrame {
		...
		private void initComponents() {
			...
			// ---- comboBox1 ----
			comboBox1.setModel(new DefaultComboBoxModel(new String[] { "Guam",
					"Marrentill", "Tarromin", "Harralander", "Ranarr",
		...
		private void startActionPerformed(final ActionEvent e) {
	...
	private class Line {
		...
		public void drawTo(final Graphics g, final Line line) {
			...
			if (calc.tileOnMap(line.getTile1())
				...
				final Point p1 = calc.tileToMinimap(t1);
				final Point p2 = calc.tileToMinimap(t2);
				final Point p3 = calc.tileToMinimap(line.getTile2());
				final Point p4 = calc.tileToMinimap(line.getTile1());
	...
	private enum State {
		CHECK_CROPS, PICK, RAKE, COMPOST, PLANT, CLEAR, CURE, WALKING, TELEPORT, BREAK
	}
	...
	private final RSArea camelotFarmingArea = new RSArea(new RSTile(2804, 3458), new RSTile(2815, 3469));
	private final RSArea faladorFarmingArea = new RSArea(new RSTile(3050, 3303), new RSTile(3061, 3315));
	private final RSArea ardougneFarmingArea = new RSArea(new RSTile(2660, 3370), new RSTile(2672, 3381));
	private final RSArea camelotTeleArea = new RSArea(new RSTile(2752, 3472), new RSTile(2762, 3482));
	private final RSArea faladorTeleArea = new RSArea(new RSTile(2957, 3374), new RSTile(2970, 3384));
	private final RSArea ardougneTeleArea = new RSArea(new RSTile(2656, 3298), new RSTile(2666, 3311));
	private final Line[] camelotPath = { new Line(2755, 3481, 2755, 3475),
			new Line(2765, 3478, 2765, 3475), new Line(2772, 3478, 2773, 3472),
			new Line(2782, 3476, 2780, 3463), new Line(2790, 3473, 2789, 3463),
			new Line(2803, 3473, 2801, 3470), new Line(2814, 3465, 2814, 3461) };
	private final Line[] faladorPath = { new Line(2960, 3384, 2962, 3376),
			new Line(2971, 3381, 2971, 3378), new Line(2983, 3378, 2982, 3373),
			new Line(2988, 3373, 2986, 3370), new Line(2993, 3370, 2992, 3369),
			new Line(2997, 3367, 2996, 3365), new Line(3007, 3361, 3001, 3352),
			new Line(3008, 3349, 3006, 3349), new Line(3008, 3340, 3006, 3340),
			new Line(3008, 3335, 3006, 3335), new Line(3008, 3324, 3005, 3324),
			new Line(3009, 3322, 3006, 3317), new Line(3020, 3322, 3020, 3314),
			new Line(3030, 3324, 3030, 3314), new Line(3042, 3322, 3042, 3314),
			new Line(3052, 3318, 3052, 3313), new Line(3055, 3309, 3060, 3309) };
	// private Line[] ardougnePath = {new Line(2659, 3296, 2669, 3305), new
	// Line(2651, 3316, 2654, 3318), new Line(2648, 3324, 2651, 3324), new
	// Line(2646, 3330, 2647, 3333), new Line(2637, 3332, 2641, 3335), new
	// Line(2633, 3343, 2638, 3344), new Line(2635, 3351, 2638, 3351), new
	// Line(2635, 3364, 2640, 3364), new Line(2641, 3375, 2645, 3372), new
	// Line(2644, 3377, 2647, 3376), new Line(2646, 3382, 2649, 3379), new
	// Line(2652, 3383, 2652, 3381), new Line(2658, 3382, 2658, 3381), new
	// Line(2669, 3377, 2670, 3373)};
	private final Line[] ardougnePath = { new Line(2653, 3311, 2653, 3298),
			new Line(2672, 3306, 2672, 3304), new Line(2690, 3307, 2692, 3305),
			new Line(2689, 3320, 2695, 3321), new Line(2688, 3328, 2690, 3328),
			new Line(2677, 3345, 2686, 3347), new Line(2675, 3358, 2686, 3363),
			new Line(2666, 3373, 2670, 3377) };
	private AntiBan antiBan;
	private Timer runTime;
	private Timer breakTimer;
	private ArrayList<RSTile> pathList;
	private boolean hidePaint = false;
	private final Rectangle closePaint = new Rectangle(425, 9, 83, 26);
	...
	private void checkCrops() {
		...
		final RSObject plot = objects.getNearest(herbPlot);
		if (plot != null) {
	...
	private boolean checkInventory() {
		return inventory.contains(seedID) && inventory.contains(rakeID)
				&& inventory.contains(dibberID) && inventory.contains(spadeID);
	}
	...
	private void clearHerbs() {
		...
		if (plot != null) {
			...
			final Timer idleTimer = new Timer(random(1250, 2000));
	...
	private void compostPatch() {
		...
		if (plot != null) {
			if (!inventory.contains(compostID)) {
				final RSNPC leprechaun = npcs.getNearest(leprechaunID);
			...
			if (inventory.contains(compostID)) {
				final int tmp = inventory.getCount(compostID);
				for (int i = 0; i < 5; i++) {
					if (tmp == inventory.getCount(compostID)
							&& currentState == State.COMPOST) {
						if (i > 0) {
							camera.setAngle(random(0, 360));
						}
						if (inventory.contains(compostID)) {
							inventory.useItem(inventory.getItem(compostID), plot);
						}
	...
	private void cureHerbs() {
		...
		if (plot != null) {
			if (inventory.contains(plantCureID)) {
				final int tmp = inventory.getCount(plantCureID);
				for (int i = 0; i < 5; i++) {
					if (tmp == inventory.getCount(plantCureID)
							&& currentState == State.CURE) {
						if (i > 0) {
							camera.setAngle(random(0, 360));
						}
						inventory.useItem(inventory.getItem(plantCureID), plot);
						sleep(random(3000, 4000));
					} else {
						break;
					}
	...
	public ArrayList<RSTile> cutUp(final ArrayList<RSTile> tiles) {
	...
	private void dropAll(final ArrayList<Integer> ids) {
	...
	private ArrayList<RSTile> generatePath(final Line[] lines) {
		final double minStep = 5, maxStep = 10, wander = 3;
		if (lines.length < 2) {
			return null;
		}
		final ArrayList<RSTile> path = new ArrayList<RSTile>();
		...
		final RSTile p = l2.translate((int) distFromCenter);
		...
		for (int i = 1; i < lines.length; i++) {
			...
			while (!finished) {
				...
				if (finished) {
					final RSTile t = l2.translateFromCenter(distFromCenter);
					curX = t.getX();
					curY = t.getY();
				} else {
	...
	private RSArea getFarmingArea(final Location loc) {
		switch (loc) {
		case CAMELOT:
	...
	private Line[] getPath(final Location loc) {
		switch (loc) {
		case CAMELOT:
			return camelotPath;
		case FALADOR:
	...
	private int getSpell(final Location loc) {
		switch (loc) {
		case CAMELOT:
	...
	private RSArea getTeleArea(final Location loc) {
		switch (loc) {
		case CAMELOT:
	...
	private void handleBreak() {
		game.logout(false);
		final int sleepLength = random(900000, 1500000); // 15-25 minutes (grow
															// a little then
															// check on them)
	...
	public int loop() {
		if (checkInventory() || status == "Logging back in") {
			if (status == "Logging back in") {
				...
				sleep(random(4000, 5000)); // Wait while information is loaded
											// from server
			}
	...
	private void noteHerbs() {
		if (inventory.containsOneOf(grimyID)) {
			...
			final int tmp = inventory.getCount(grimyID);
			for (int i = 0; i < 5; i++) {
	...
	public void onRepaint(final Graphics g1) {
		...
		if (hidePaint) {
			...
			final int profit = marketPriceOfHerbs * numHerbsFarmed
					- marketPriceOfSeeds * numSeedsPlanted;
			...
			g.setColor(color3);
			g.drawString(skillPercent + "% to level " + nextLevel + " ("
					+ xpToLevel + " xp)", 216, 334);
			g.setColor(color1);
	...
	public boolean onStart() {
		...
		final HerbFarmerGUI GUI = new HerbFarmerGUI();
		GUI.setVisible(true);
		while (GUI.isVisible()) {
			sleep(50);
		}
		...
		log("The market price of each " + seedType + " seed is: "
				+ marketPriceOfSeeds + "gp");
		log("The market price of each " + seedType + " herb is: "
				+ marketPriceOfHerbs + "gp");
	...
	private void performMaintainance() {
		final ArrayList<Integer> toDrop = new ArrayList<Integer>();
		toDrop.add(weedsID);
		toDrop.add(bucketID);
		toDrop.add(vialID);
		dropAll(toDrop);
	...
	private void plantSeed() {
		...
		if (plot != null && inventory.contains(seedID)) {
			final int tmp = inventory.getItem(seedID).getStackSize();
			for (int i = 0; i < 5; i++) {
				if (tmp == inventory.getItem(seedID).getStackSize()
						&& currentState == State.PLANT) {
					if (i > 0) {
						camera.setAngle(random(0, 360));
					}
					inventory.useItem(inventory.getItem(seedID), plot);
	...
	private int step(final ArrayList<RSTile> path) {
		...
		if (index >= 0
				&& (dest == null || index > shortest || !getMyPlayer().isMoving())) {
	...
	private void teleport(final Location loc) {
		status = "Teleporting to " + loc.toString();
		if (magic.castSpell(getSpell(loc))) {
			sleep(random(9000, 11000));
			if (getTeleArea(loc).contains(getMyPlayer().getLocation())) { // Teleport
																			// successful
																			// if
																			// player
																			// is
	...
	private boolean tileInNextRange(final RSTile t) {
		return calc.distanceBetween(t, getMyPlayer().getLocation()) < nextStep;
	}

	private void updateHerbCount() {
		if (inventory.contains(notedID)) {
			numHerbsFarmed = inventory.getItem(notedID).getStackSize()
					- startingNoteSize;
		} else {
			numHerbsFarmed = 0;
		}
	...
	private void walk(final Location loc, final State nextState) {
		if (getFarmingArea(loc).contains(getMyPlayer().getLocation())) {
			currentLocation = loc;
			currentState = nextState; // This is the last spot of each run.
										// Break here.
		} else {
			checkEnergy();
