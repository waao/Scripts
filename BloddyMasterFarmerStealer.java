...
public class BloddyMasterFarmerStealer extends Script implements PaintListener,
	...
	public class BloddyMasterFarmerGUI extends JFrame {
		...
		private void checkBox2ActionPerformed(final ActionEvent e) {
		...
		private void checkBox3ActionPerformed(final ActionEvent e) {
	...
	public class cameraHeight extends Thread {
		...
		public void run() {
			while (isActive) {
				if (Methods.random(1, 25000) == 342) {
					try {
						char key = KeyEvent.VK_UP;
						if (camera.getPitch() >= Methods.random(35, 100)) {
							key = KeyEvent.VK_DOWN;
						}
						keyboard.pressKey(key);
						Thread.sleep(Methods.random(75, 500));
			...
			while (isActive) {
				if (Methods.random(1, 25000) == 342) {
					try {
						...
						keyboard.pressKey(key);
						Thread.sleep(Methods.random(350, 1300));
						keyboard.releaseKey(key);
					} catch (final Exception e) {
	...
	private final boolean highLevelMode = false;
	...
	private int FOODID;
	private final int ANIMATIONID = 11974;
	...
	public boolean antiBan() {
		final int randomNumber = Methods.random(1, 16);
		if (randomNumber <= 16) {
			if (randomNumber == 1) {
				openRandomTab();
	...
	public boolean antiBan2() {
		final int randomNumber = Methods.random(1, 45);
		if (randomNumber <= 9) {
			if (randomNumber == 1) {
			}
			if (randomNumber == 2) {
	...
	private int checkEat() {
		final int CurrHP = getCurrentLifepoint() / 10;
		final int RealHP = skills.getRealLevel(Skills.CONSTITUTION);
		if (CurrHP <= Methods.random(RealHP / 2, RealHP / 1.5)) {
			if (!highLevelMode) {
				status = "Eating Food";
	...
	public int checkInventoryFull() {
		if (!highLevelMode) {
			if (bank.isOpen() && bank.getCount(FOODID) == 0
				...
				log("Out of food! Logging out!");
				Methods.sleep(Methods.random(4000, 5000));
				bank.close();
				logOut();
			}
		}
				...
				if (!atBank()) {
					...
					if (calc.distanceTo(walking.getDestination()) < Methods
						...
						if (!walking.walkPathMM(farmerToBank)) {
							...
							return Methods.random(1400, 1600);
						}
					}
				} else if (atBank()) {
					openBank();
	...
	public boolean doSomethingNPC(final int id, final String action) {
		final org.rsbot.script.wrappers.RSNPC NPC = npcs.getNearest(id);
		if (NPC == null) {
			return antiBan();
		}
		if (NPC.isOnScreen()) {
	...
	public void onRepaint(final Graphics g) {
		...
		if (game.getCurrentTab() == Game.TAB_INVENTORY) {
			if (showInventory == false) {
				...
				final int[] coords = new int[] { 225, 240, 255, 270, 285, 300,
	...
	private void openRandomTab() {
		final int randomNumber = Methods.random(1, 14);
		if (randomNumber <= 14) {
			if (randomNumber == 1) {
				game.openTab(Game.TAB_STATS);
			}
