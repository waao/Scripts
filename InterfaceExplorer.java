...
public class InterfaceExplorer extends Script implements PaintListener {
	...
	public Component getTreeCellRendererComponent(final JTree tree,
		...
		for (int i = 0; i < HighLightableWraps.size(); i++) {
			...
			/*
			 * if (NextInterface.getComp().getID() ==
			 * ((RSComponentWrap) value).wrapped.getID()) {
			 * //log(((RSComponentWrap) value).wrapped.getIndex());
			 * } if (NextInterface.getChild().getID() ==
			 * ((RSComponentWrap) value).wrapped.getID()) {
			 * //log(((RSComponentWrap) value).wrapped.getIndex());
			 * }
			 */
	...
	if (a.getComponent().getArea() != null
		&& a.getArea() != null
		&& a.getComponent().getArea().getSize()
			.equals(a.getArea().getSize())) {
	    HighLightWraps.add(new HighLightInterface(a.getComponent()
		    .getParent().getInterface(), a.getComponent().getParent(),
		    a.getComponent(), 15));
	}
	...
	tree.addTreeSelectionListener(new TreeSelectionListener() {
		...
		if (iface.getArea() != null) {
		    ...
		    addInfo("Size: ", size.width + "," + size.height,
			    changes.contains(15));
		}
		...
		addInfo("Rotation: ",
			"x: " + iface.getXRotation() + "  y: "
				+ iface.getYRotation() + "  z: "
				+ iface.getZRotation(), changes.contains(16));
