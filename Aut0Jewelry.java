...
public class Aut0Jewelry extends Script implements PaintListener, MouseListener {
	...
	public class GUI extends JFrame {
...

				{
...

					{
						...
						// @START - TAB 1
						{
							...
							// @START CRAFTINGTYPE
							craftingType.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
							...
							craftingType.addActionListener(new ActionListener() {
								...
								public void actionPerformed(
									...
									if (co == "Gold Jewellery") {
										craftObj.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
							...
							mouseSpeed.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
									"Slow", "Slow-Medium",
									"Medium(Recommended)",
									"Medium-Fast(Recommended)", "Fast**",
									"Super Fast**" }));
						...
						// ======== panel2 ========
						{
							...
							panel2.add(label3);
							label3.setBounds(5, 5, label3.getPreferredSize().width, 15);

							// ---- label4 ----
							label4.setText("Ver." + version);
							label4.setFont(new Font("Cambria", Font.BOLD, 10));
							panel2.add(label4);
							label4.setBounds(5, 15, label4.getPreferredSize().width, 20);
					...
					{ // compute preferred size
						...
						for (int i = 0; i < contentPanel.getComponentCount(); i++) {
							final Rectangle bounds = contentPanel.getComponent(i).getBounds();
							preferredSize.width = Math.max(bounds.x
									+ bounds.width, preferredSize.width);
							preferredSize.height = Math.max(bounds.y
									+ bounds.height, preferredSize.height);
						}
						final Insets insets = contentPanel.getInsets();
						preferredSize.width += insets.right;
						preferredSize.height += insets.bottom;
		...
		private void StartActionPerformed(final java.awt.event.ActionEvent evt) {
			final String location = (String) loc.getSelectedItem();
			final String uo = (String) type.getSelectedItem();
			final String co = (String) craftingType.getSelectedItem();
			final String go = (String) craftObj.getSelectedItem();
			final String speed = (String) mouseSpeed.getSelectedItem();

			if (speed.equals("Slow")) {
				speedMouse = Methods.random(10, 11);
			} else if (speed.equals("Slow-Medium")) {
				speedMouse = Methods.random(7, 10);
			} else if (speed.equals("Medium(Recommended)")) {
				speedMouse = Methods.random(6, 7);
			} else if (speed.equals("Medium-Fast(Recommended)")) {
				speedMouse = Methods.random(5, 6);
			} else if (speed.equals("Fast**")) {
				speedMouse = Methods.random(3, 5);
			} else if (speed.equals("Super Fast**")) {
				speedMouse = Methods.random(2, 3);
			}
	...
	public int checkBank(final int itemID, final int amountToWithdraw) {
		if (checkItem(itemID)) {
	...
	public boolean checkItem(final int i) {
		if (bank.isOpen() && bank.getCount(i) <= 1) {
			Methods.sleep(Methods.random(25, 100));
			if (bank.isOpen() && bank.getCount(i) <= 1) {
	...
	public boolean doActionAtModel(final RSModel model, final String action) {
		if (model != null) {
			int iters = Methods.random(10, 15);
	...
	public RSTile examineRandomObject(final int scans) {
		...
		if (possibleTiles.size() > 0 && possibleTiles != null) {
			final RSTile objectLoc = possibleTiles.get(Methods.random(0, possibleTiles.size()));
	...
	private ArrayList<RSTile> generatePath(final Line[] lines) { // you're
																	// generating
		// the path at
		// walking start,
		// you need to
		...
		path = new ArrayList<RSTile>();
		Line l1, l2 = lines[0];
		double distFromCenter = Methods.random(0, l2.getDistance() + 1);
		...
		for (int i = 1; i < lines.length; i++) {
			...
			finished = false;
			while (!finished) {
				distOnLine += Methods.random(minStep, maxStep);
				...
				if (force1 < 0) {
					...
					distFromCenter += Methods.random(-cap1, cap2);
				}
				if (finished) {
				...
				for (int j = 0; j < dist; j++) {
					if (objects.getTopAt(new RSTile((int) Math.round(xdist * j
						...
						RSTile ran = null;
						ran = new RSTile((int) Math.round(xdist * j + lastX) + 1, (int) Math.round(ydist
								* j + lastY));
						...
						ran = new RSTile((int) Math.round(xdist * j + lastX), (int) Math.round(ydist
								* j + lastY) + 1);
						if (objects.getTopAt(ran) == null) {
							path.add(ran);
							continue;
	...
	public int getDistance2(final RSTile t) {
		return Math.abs(getMyPlayer().getLocation().getX() - t.getX())
				+ Math.abs(getMyPlayer().getLocation().getY() - t.getY());
	}
	...
	public State getState() {
		if (globalBanking) {
			...
			final int randAntiBanGen = Methods.random(1, 40);
	...
	public void hoverObject() {
		try {
			...
			Methods.sleep(Methods.random(50, 1000));
			final int mousemoveAfter2 = Methods.random(0, 4);
			Methods.sleep(Methods.random(100, 800));
	...
	boolean hoverPlayer() {
		...
		player = validPlayers[Methods.random(0, validPlayers.length - 1)];
		if (player != null) {
			try {
	...
	public int loop() {
		try {
			if (bank.isOpen()) {
				if (inventory.containsOneOf(Aut0Jewelry.BOX_ID, Aut0Jewelry.BOOK_KNOWLEDGE_ID, Aut0Jewelry.LAMP_ID, Aut0Jewelry.MYSTERY_BOX_ID)) {
					bank.close();
			...
			final RSObject banker = objects.getNearest(bankID);
			...
			case CUTTING:
				...
				if (!isCutting()
					...
					interfaces.getComponent(ifParent, ifChild).doAction("Cut All");
				}
				if (isCutting() && inventory.contains(uncutGemID)) {
					...
					final RSItem RSI_uncut = inventory.getItem(uncutGemID);
					final RSItem RSI_chisel = inventory.getItem(chiselID);
					if (RSI_uncut != null && RSI_uncut != null
						...
						if (inventory.isItemSelected()) {
							if (RSI_uncut != null) {
								...
								waitForIF(interfaces.get(ifParent), Methods.random(2000, 3000));
							}
						}
						Methods.sleep(Methods.random(300, 600));
					}
			...
			case CRAFT:
				...
				if (interfaces.getComponent(ifParent, ifChild).isValid()) {
					...
					if (!gem) {
						Methods.sleep(Methods.random(750, 1000));
					} else if (gem) {
						Methods.sleep(Methods.random(500, 750));
					}
				} else if (isIdle()
						&& !interfaces.getComponent(ifParent, ifChild).isValid()) {
					final RSObject smithObj = objects.getNearest(smithID);
					final RSItem itemOnFurn = inventory.getItem(barID);
					if (smithObj != null && itemOnFurn != null
						...
						final RSModel furModel = smithObj.getModel();
						if (furModel != null) {
							if (failCounter > 2) {
								failCounter = 0;
			...
			case ANTIBAN:
				try {
					final int ran = Methods.random(0, 6);
					switch (ran) {
					case 0:
						final int cam = Methods.random(0, 3);
						switch (cam) {
						case 0:
					...
					case 1:
						...
						hoverPlayer();
						Methods.sleep(Methods.random(550, 1800));
					...
					case 3:
						...
						if (System.currentTimeMillis() - mouse.getPressTime() > 5000) {
							final Point mouseP = mouse.getLocation();
	...
	@Override
	public void onRepaint(final Graphics a) {
		final int currentExp = skills.getCurrentExp(Skills.CRAFTING);
		final int currentLvl = skills.getCurrentLevel(Skills.CRAFTING);
		gainedXp = currentExp - startExp;
		gainedLvl = currentLvl - startLvl;
		itemsMade = gainedXp / exp;
		final int cutPerHour = (int) (itemsMade * 3600000D / (System.currentTimeMillis() - startTime));
		final int expPerHour = (int) (gainedXp * 3600000D / (System.currentTimeMillis() - startTime));
		double profit, r__pph;
		if (gem) {
			profit = (double) (itemsMade * itemsPrice - (barPrice * itemsMade + gemPrice
					* itemsMade)) / 1000;
			r__pph = itemsMade
					* itemsPrice
					* 3600000D
					/ (System.currentTimeMillis() - startTime - +cutPerHour
							* gemPrice + +cutPerHour * barPrice) / 1000;
		} else {
		...
		final long yr__pph = (long) (r__pph * 100);
		final double profitPerHour = (double) yr__pph / 100;
		...
		final Graphics2D g = (Graphics2D) a;
		if (!globalBanking) {
		...
		final int currentCraftingLevel = skills.getCurrentLevel(Skills.CRAFTING);
		final int percentToNextCraftingLevel = skills.getPercentToNextLevel(Skills.CRAFTING);
		final float t = skills.getPercentToNextLevel(Skills.CRAFTING);
		final float h = t / 100;
	...
	private int step(final ArrayList<RSTile> path) {
		...
		if (index >= 0
			...
			nextStep = Methods.random(13, 15);
			while (walking.getDestination() != null
					&& calc.distanceBetween(walking.getDestination(), getMyPlayer().getLocation()) >= 5) {
	...
	public boolean waitForBankIF(final int timeout) {
	...
	private boolean waitForDepositedItem(final int timeout) {
		final long start = System.currentTimeMillis();
	...
	public boolean waitForEnableRun(final int timeout) {
		final long start = System.currentTimeMillis();
		while (System.currentTimeMillis() - start < timeout) {
	...
	public boolean waitForIF(final RSInterface iface, final int timeout) {
		final long start = System.currentTimeMillis();
	...
	public boolean waitForInventoryAction(final int timeout) {
		final long start = System.currentTimeMillis();
	...
	private boolean waitForWithdrawnItem(final int itemID, final int timeout) {
		final int startCount = inventory.getCount(itemID);
		final long start = System.currentTimeMillis();
