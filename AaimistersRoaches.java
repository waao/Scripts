...
public class AaimistersRoaches  extends Script implements PaintListener, MouseListener, MessageListener {
	...
	private final String[] lootstring = { "Coins", "Mushroom", "Nature_Tail", "Shield_Left_Half", "Loop_Half_Key", "Tooth_Half_Key", "JangerBerry", 
										   "Wildblood", "Limpwurt", "Avantoe", "Belladonna", "Cactus", "Cadantine", "Irit", "Kwuarm", "Marrentill", 
										   "Mushroom", "Poison_Ivy", "Strawberry", "Tarromin", "Toadflax", "Whiteberry", "Watermelon", "Harralander",
										   "Spirit_Weed", "Dwarf_Weed", "Lantadyme", "Snapdragon", "Torstol", "Ranarr", "All Charms", "Sapphire", 
										   "Emerald", "Ruby", "Diamond", "Dragonstone", "Mithril_Ore", "Law_Rune", "Death_Rune", "Adamantite_Ore", 
										   "Fire_Rune", "Blood_Rune", "Chaos_Rune", "Black_Helm", "Mithril_Helm", "Rune_Sq_Shield", "Rune_Scimi", 
										   "Rune_Javelin", "Dragon_Spear", "Rune_Spear" };
	private final int[] lootint = { 995, 6004, 1462, 2366, 987, 985, 5104, 5311, 5100, 
									5298, 5281, 5280, 5301, 5297, 5299, 5292, 5282, 5106, 5323, 
									5293, 5296, 5105, 5321, 5294, 12176, 5303, 5302, 5300, 5304, 
									5295, 0000, 1623, 1621, 1619, 1617, 1631, 448, 563, 560, 
									450, 554, 565, 562, 1165, 1143, 1185, 1333, 830, 1249, 1247 };
	...
	int Aitems[] = { 890, 882, 11212, 19157, 884, 888, 2866, 892, 19152, 
					 886, 19162 };
	//Coins, Mushroom, Clue Scrool (m), Nature Tail, Shield Left Half, Loop Half Key, Tooth half Key 
	...
	//Black helm, Mithril med, Rune sq, Rune scimi, Rune javelin, Dragon spear, Rune spear
	int Witems[] = { 1165, 1143, 1185, 1333, 830, 1249, 1247 };
	//All
	int Zitems[] = { 995, 6004, 1462, 2366, 987, 985, 5104, 5311, 5100, 
			5298, 5281, 5280, 5301, 5297, 5299, 5292, 5282, 5106, 5323, 
			5293, 5296, 5105, 5321, 5294, 12176, 5303, 5302, 5300, 5304, 
			5295, 12160, 12163, 12158, 12159, 12161, 12162, 12164, 12165, 
			12166, 12167, 1623, 1621, 1619, 1617, 1631, 448, 563, 560, 
			450, 554, 565, 562, 1165, 1143, 1185, 1333, 830, 1249, 1247 };
	...
	private boolean loot() {
		RSGroundItem a = groundItems.getNearest(Aitems);
		RSGroundItem z = groundItems.getNearest(Zitems);
		if (z != null && rArea.contains(z.getLocation()) && getMyPlayer().getInteracting() == null) {
			return true;
		} else if (rgxpGained != 0 && a != null && rArea.contains(a.getLocation()) && getMyPlayer().getInteracting() == null) {
			return true;
		} else {
			return false;
		}
	...
	private void lootItem(RSGroundItem x) {
		try {
	...
	public int loop() {
		...
		case BANK:
			...
			if (useBanker) {
				RSNPC bankP = banker();
				if (AM.bankArea.contains(getMyPlayer().getLocation()) && calc.tileOnScreen(banker().getLocation())) {
					if (!bank.isOpen()) {
						idle++;
						if (!opened) {
		...
		case LOOT:
			...
			RSGroundItem a = groundItems.getNearest(Aitems);
			RSGroundItem o = groundItems.getNearest(Oitems);
			RSGroundItem s = groundItems.getNearest(Sitems);
			RSGroundItem c = groundItems.getNearest(Citems);
			RSGroundItem g = groundItems.getNearest(Gitems);
			RSGroundItem r = groundItems.getNearest(Ritems);
			RSGroundItem w = groundItems.getNearest(Witems);
			if (rgxpGained != 0) {
	 ...
	 public class AaimistersGUI {
		...
		private AaimistersGUI() {
	   	...
	   	public void initComponents() {
			...
			JLabel lblAaimistersRoachKiller = new JLabel("Aaimister's Roach Killer v1.11");
			...
			gl_contentPane.setHorizontalGroup(
				gl_contentPane.createParallelGroup(Alignment.LEADING)
					.addComponent(lblAaimistersRoachKiller, GroupLayout.DEFAULT_SIZE, 434, Short.MAX_VALUE)
