...
public class YakAttack extends Script implements PaintListener, MessageListener {
	...
	public class YakAttackProGUI extends JFrame {
		...
		private void ExitActionPerformed(final ActionEvent e) {
			guiWait = false;
			guiExit = true;
			dispose();
		}
	...
	private String buildString(final int paramInt) {
		...
		double d;
		if (paramInt >= 10000) {
			d = YakAttack.round(paramInt / 1000.0D, 1);
			return new StringBuilder().append(d).append("K").toString();
		}
		if (paramInt >= 1000) {
			d = YakAttack.round(paramInt / 1000.0D, 2);
			return new StringBuilder().append(d).append("K").toString();
		}
		return new StringBuilder().append(paramInt).append("").toString();
	}

	private Point centerPoint(final RSModel o, final int level) {
		if (o == null) {
			return new Point(-1, -1);
		}
		int xTotal = 0;
		int yTotal = 0;
		final Point[] points = new Point[level];
	...
	private void drawItem(final Graphics g, final RSItem item,
			final Boolean food) throws InterruptedException {
		if (food) {
	...
	private void drawNiceBox(final Graphics g, final String text, final int x,
			final int y, final Color col1, final Color col2, final Color col3,
	...
	private int getAction() {
		...
		for (final Object[] tileinfo : validTiles) {
			final RSTile tile = (RSTile) tileinfo[1];
			if (calc.distanceTo(tile) < 50) {
				if (!YakAttack.currLocation.equals(tileinfo[0])) {
					YakAttack.currLocation = (String) tileinfo[0];
				}
				return KILLYAKS;
			}
		}
	...
	private int getAngleToCoord(final RSTile loc) {
		final int x1 = getMyPlayer().getLocation().getX();
		final int y1 = getMyPlayer().getLocation().getY();
		final int x = x1 - loc.getX();
		final int y = y1 - loc.getY();
	 ...
	 */
	@SuppressWarnings("unused")
	private RSNPC getNearestFree(final int... ids) {
		int Dist = 20;
		RSNPC closest = null;
		final RSNPC[] validNPCs = npcs.getAll();

		final RSNPC Monster = validNPCs[Methods.random(0, validNPCs.length)];
		try {
			for (final int id : ids) {
				if (id != Monster.getID() || Monster.isInCombat()) {
					continue;
				}
				final int distance = calc.distanceTo(Monster);
				if (distance < Dist) {
					Dist = distance;
					closest = Monster;
				}
	...
	private void getRndSpec() {
		rndSpec = Methods.random(specialCost, 1000);
		log("Will next Spec at " + rndSpec / 10 + "%");
	}

	private boolean handleArrows() {
		final RSGroundItem loots = groundItems.getNearest(arrowPack);
	...
	private boolean listContainsString(final String[] list, final String string) {
		try {
			int a;
	...
	private String locToString() {
		return YakAttack.currLocation;
	}

	@Override
	public int loop() {
		foodInBag = inventory.getItem(foodID);
		final RSNPC yak = npcs.getNearest(new Filter<RSNPC>() {
			@Override
		...
		if (wants2Eat) {
			if (combat.getLifePoints() <= Methods.random(hp2EatAt - 30,
				...
				if (inventory.contains(Food)) {
					...
					return Methods.random(800, 1400);
					...
					if (getMyPlayer().isInCombat()) {
						Methods.sleep(Methods.random(10000, 11000));
					}
					stopScript(true);
				}
			}
		...
		if (doSpec) {
			if (rndSpecCtr == 0 && settings.getSetting(300) >= specialCost) {
				if (settings.getSetting(301) != 1) {
					...
					final int tempRndSpec = settings.getSetting(300)
							- specialCost;
	...
	public void messageReceived(final MessageEvent e) {
		...
		if (serverString.contains("already under attack")) {
			Methods.sleep(Methods.random(2500, 4000));
		}
	...
	public void onRepaint(final Graphics g) {
		...
		final int mouse_press_y = mouse.getPressLocation().y;
		...
		final long l1 = timeRan.getElapsed();
		final long l2 = l1 / 1000L;
		final long l3 = l2 / 60L;
		final long l4 = l3 / 60L;
		final long l5 = l4 / 24L;
		final int j = (int) l2 % 60;
		final int k = (int) l3 % 60;
		final int m = (int) l4 % 60;
		final int n = (int) l5;
		if (n > 3) {
		...
		int i2 = 0;
		String firstSet = "";
		final StringBuilder localStringBuilderNew = new StringBuilder();

		if (l2 > 0L) {
			for (int i3 = 0; i3 < diffexp.length; i3++) {
				if (diffexp[i3] > 0) {
					final int i4 = diffexp[i3] * 60 * 60 / (int) l2;
					final int i5 = exptolevel[currentlvl[i3]] - currentexp[i3];
					final StringBuilder localStringBuilder3 = new StringBuilder();
					final long l6 = i5 * timeRan.getElapsed() / diffexp[i3];

					final long l7 = l6 / 1000L;
					final long l8 = l7 / 60L;
					final long l9 = l8 / 60L;
					final long l10 = l9 / 24L;
					final int i17 = (int) l7 % 60;
					final int i18 = (int) l8 % 60;
					final int i19 = (int) l9 % 60;
					final int i20 = (int) l10;
					if (i20 > 3) {
		...
		final int xpHour = (int) (3600000.0D / timeRan.getElapsed() * i2);
		yaksKilled = i2 / 200;
	...
	private boolean pickupArrows(final int[] id, final String itemName) {
		boolean back = false;
		try {
			final RSGroundItem loots = groundItems.getNearest(id);
			final Point toscreen = calc.tileToScreen(loots.getLocation());
	...
	private void runControl() {
		if (!walking.isRunEnabled()
				&& walking.getEnergy() > Methods.random(20, 30)) {
			walking.setRun(true);
		}
	}
