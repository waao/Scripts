...
public class AionAirCrafter extends Script implements PaintListener,
	...
	public class BankAction extends Action {
		...
		public void process() {
			if (bank.isOpen()) {
				...
				Methods.sleep(600, 800);
			} else {
				final RSNPC banker = npcs.getNearest(Constants.FILTER_NPC);
				if (banker != null) {
	...
	public abstract class ObjectAction extends Action {
		...
		public ObjectAction(final String action, final int id) {
			this.action = action;
			this.id = id;
		}
		...
		@Override
		public void process() {
			final RSObject object = objects.getNearest(id);
			if (object != null) {
	...
	public abstract class WalkToArea extends Action {
		...
		public WalkToArea(final RSPath path) {
			this.path = path;
		}
	...
	private void antiban() {
		...
		case 2:
			if (Methods.random(1, 5) != 1) {
				break;
			}
			mouse.moveSlightly();
			break;
		case 6:
			if (Methods.random(1, 18) != 7) {
				break;
			}
			...
			Methods.sleep(Methods.random(1400, 2000), 3000);
			if (Methods.random(0, 5) != 3) {
				break;
			}
			mouse.moveSlightly();
			break;
		...
		case 17:
		case 25:
			camera.setAngle(Methods.random(-360, 360));
			break;
		case 30:
		case 34:
		case 37:
	...
	private double getExpHour() {
		final int xpGained = getExpGained();
		final long start = System.currentTimeMillis() - timer.getElapsed();
	...
	private double getRunesHour() {
		final int runes = getRunesCrafted();
		final long start = System.currentTimeMillis() - timer.getElapsed();
	...
	private Point getScreenPoint(final RSObject obj) {
		if (obj != null) {
	...
	private String getTimeFormat(final long time) {
	...
	public int loop() {
		if (interfaces.get(741).isValid()) {
			...
			if (game.getCurrentTab() != Game.TAB_STATS) {
				game.openTab(Game.TAB_STATS);
				Methods.sleep(800, 1200);
			}
			skills.doHover(Skills.INTERFACE_RUNECRAFTING);
			Methods.sleep(150, 300);
			mouse.click(true);
			return Methods.random(Methods.random(1000, 1300), 2000);
		}
		...
		if (Methods.random(1, 10) == Methods.random(1, 6)) {
			antiban();
		}
	...
	public void onRepaint(final Graphics g) {
		...
		final Point mPoint = mouse.getLocation();
		final Point pPoint = mouse.getPressLocation();
	...
	private void waitForAnim(final int timeout) {
	...
	private void waitForIface(final int iface, final int timeout) {
		final long endTime = System.currentTimeMillis() + timeout;
		while (System.currentTimeMillis() < endTime) {
