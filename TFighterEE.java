 ...
 * v1.60: Stable: Potions and ash scattering added.
...
public class TFighterEE extends Script implements PaintListener, MouseListener {
	...
	public boolean onStart() {
		...
		LoopAction[] actions;
		if(prioritizeLoot) {
			actions = new LoopAction[] { (buryBones ? new BonesLoop() : null), new LootLoop(), (useSafespot ? new SafespotLoop() : null),  new InCombatLoop(), new AttackLoop() };
		} 
		else {
			actions = new LoopAction[] { (useSafespot ? new SafespotLoop() : null), new InCombatLoop(), (buryBones ? new BonesLoop() : null), new LootLoop(),  new AttackLoop() };
		}
	...
	private class BonesLoop implements LoopAction {
		...
		public int loop() {
			for (RSItem item : inventory.getItems(BONE_IDS)) {
				item.doClick(true);
	...
	private class Potion {
		...
		public int usePotions() {
			...
			if( (skills.getRealLevel(Skills.PRAYER) - skills.getCurrentLevel(Skills.PRAYER) >= random( Math.floor(7 + skills.getRealLevel(Skills.PRAYER) / 4) - 2 , Math.floor(7 +(skills.getRealLevel(Skills.PRAYER) / 4))  + 2)) && potions.get("PRAYER").length != 0) {
				return random(2000, 2500);
			}
	...
	private class FighterGUI extends JFrame {
		...
		private JCheckBox useMulti, useRadius, useSafe, useCentral, prioritizeLoot, useBones;
		...
		private void init() {
			...
			{
				JLabel title = new JLabel("TFighterEE");
			...
			{
				JLabel mouseSpeedLabel = new JLabel("Enter your desired mouse speed (max,min)");
				mouseSpeedBox = new JTextField("4,7");
				
				JLabel npcLabel1 = new JLabel("Enter the IDs and/or names of the NPCs to fight.");
				JLabel npcLabel2 = new JLabel("You can mix and match these, all in the same box!");
				npcBox = new JTextField("2,5,1,Chicke");
				
				JLabel lootLabel = new JLabel("Enter the IDs and/or names of items to loot.");
				...
				JLabel prioritizeLabel = new JLabel("If selected, you will loot while in combat.");
