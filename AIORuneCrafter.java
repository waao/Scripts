...
public class AIORuneCrafter extends Script implements PaintListener,
	...
	private int Rune = 556, Tiara = 5527, bankBoothID = 782, ruinsID = 2452,
			altarID = 2478, portalID = 2465, musicianID = 8699;
			...
			for (int i = 0; i < 25; i++) {
				while (!menu.contains(action)) {
					if (players.getMyPlayer().isMoving()) {
						...
						p = npc.getModel().getPoint();
					}
					if (!isOnScreen(npc.getModel()) || bank.isOpen()) {
						return false;
					}
					moveMouse(getDelay(p), p);
				}
	...
	private boolean clickObject(final RSObject object, final String action) {
		try {
			...
			for (int i = 0; i < 25; i++) {
				while (!menu.contains(action)) {
					if (players.getMyPlayer().isMoving()) {
						p = object.getModel().getCentralPoint();
					} else {
						p = object.getModel().getPoint();
					}
