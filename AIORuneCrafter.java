...
@ScriptManifest(authors = { "Ubuntu4life" }, name = "AIORuneCrafter", version = 0.5, description = "Crafts all F2P runes.", website = "http://www.powerbot.org/vb/showthread.php?t=564872", requiresVersion = 244)
...
MessageListener, MouseListener {
	...
	public class AntiBan extends Thread {
		...
		public void run() {
			...
			try {
				while (!stoppingScript) {
					if (!isPaused() && game.isLoggedIn()) {
						if (random.nextInt(Math.abs(0 - 23)) == 0) {
							...
							Thread.sleep(random.nextInt(Math.abs(2004 - 198)));
						}
					}
				}
	...
	public class FreeMemory extends Thread {

		@Override
		public void run() {
			while (!stoppingScript && game.isLoggedIn()) {
				try {
					if (Methods.random(0, 350) == 1) {
	...
	private final Color lineColor = new Color(76, 13, 13),
	fillColor = new Color(0, 0, 0, 50), Succes = new Color(0, 255, 0),
		...
		try {
			...
			for (int i = 0; i < 25; i++) {
				...
				if (menu.isOpen() && !menu.contains(action)) {
					do {
						mouse.moveRandomly(Methods.random(180, 310));
					} while (menu.isOpen());
					i = 0;
				}
			}
	...
	public int loop() {
		try {
			...
			if (!walking.isRunEnabled()
					...
					&& !bank.isOpen()) {
				walking.setRun(true);
				Methods.sleep(Methods.random(250, 400));
		...
		if (game.isLoggedIn() && !game.isLoginScreen() && !stoppingScript) {
			...
			* essenceUsed;
			...
			if (tab == 1) { // General
				...
				drawString(g2, "Time until level: " + timeToLevel2, x, 433);
				...
				drawString(g2, "Essence until level: "
						+ getFormattedMoney(essTillLevel), 275, 433);
				drawString(g2, "XP until level: "
						+ getFormattedMoney(epxTillLevel), 275, 449);
				if (runesCrafted > 0) {
					final double runeRatio = runesCrafted / essenceUsed;
