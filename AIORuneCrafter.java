...
public class AIORuneCrafter extends Script implements PaintListener,
		...
		public void run() {
			...
			try {
				while (game.isLoggedIn() && !stoppingScript && !isPaused()) {
					final int rand = randomGenerator.nextInt(23);
					if (rand == 1) {
	...
	public final class PerfectGUI extends JFrame implements ActionListener {
		...
		private void Start(final ActionEvent e) {
			setVisible(false);
			startScript = true;
		}
	}
	...
	public static String getFormattedMoney(final long money) {
	...
	public static String getFormattedTime(final long timeMillis,
			final String sep, final boolean millisInc) {
	...
	private final int[] bankerID = { 553, 2759 };
	private int bankBoothID = 782;
	private final int chestID = 27663, runeEssence = 1436;
	private int Rune = 556, Tiara = 5527, ruinsID = 2452, altarID = 2478,
			portalID = 2465, musicianID = 8699;
	private final int stairsID = 24353;
	private int runesCrafted = 0, essenceUsed = 0, essenceInBank = 0, tab = 1,
	...
	RSTile altarWalkTile = new RSTile(2843, 4832),
			...
			errorTile2 = new RSTile(3257, 3419);// Two tiles
	...
	private final Line[] TOBANK = { // Fire is special
	new Line(3323, 3247, 3323, 3248), new Line(3325, 3247, 3325, 3264),
			new Line(3328, 3264, 3328, 3269), new Line(3335, 3265, 3335, 3270),
			new Line(3348, 3265, 3348, 3271), new Line(3353, 3262, 3353, 3267),
			new Line(3367, 3262, 3367, 3269), new Line(3372, 3264, 3372, 3267),
			new Line(3380, 3265, 3380, 3266), new Line(3382, 3267, 3382, 3272),
			new Line(3383, 3267, 3383, 3272) };

	private Line[] TORUINS = { // Air
	new Line(3182, 3435, 3182, 3440), new Line(3173, 3425, 3173, 3433),
			new Line(3164, 3418, 3163, 3425), new Line(3153, 3415, 3153, 3425),
			new Line(3145, 3413, 3145, 3418), new Line(3136, 3403, 3136, 3417),
	...
	private void antiBan() {
		final int random = Methods.random(1, 9);
	...
	private boolean clickNPC(final RSNPC npc, final String action) {
		try {
			Point p;
			for (int i = 0; i < 15; i++) {
				for (p = players.getMyPlayer().isMoving() ? npc.getModel().getCentralPoint()
						: npc.getModel().getPoint(); !menu.contains(action); p = players.getMyPlayer().isMoving() ? npc.getModel().getCentralPoint()
	...
	private boolean clickObject(final RSObject object, final String action) {
		try {
			Point p;
			for (int i = 0; i < 15; i++) {
				for (p = players.getMyPlayer().isMoving() ? object.getModel().getCentralPoint()
						: object.getModel().getPoint(); !menu.contains(action); p = players.getMyPlayer().isMoving() ? object.getModel().getCentralPoint()
	...
	private boolean didMisClick() {
		Methods.sleep(1);
		try {
			final Robot colorSampler = new Robot();
			final Point mousePos = mouse.getPressLocation();
			final Color pixelColor = colorSampler.getPixelColor((int) mousePos.getX() - 1, (int) mousePos.getY() + 1);
			final Color misClick = new Color(254, 254, 0);
	...
	private void drawString(final Graphics g2, final String s, final int x,
			final int y) {
	...
	private ArrayList<RSTile> generatePath(final Line[] lines) {
		final double minStep = 3, maxStep = 16;
		final int wander = Methods.random(3, 6);
		if (lines.length < 2) {
			return null;
		}
		final ArrayList<RSTile> path = new ArrayList<RSTile>();
	...
	private java.util.Random generateSeededRandom(final String name) {
		MessageDigest coder;
		try {
			coder = MessageDigest.getInstance("MD5");
			coder.update(name.getBytes());
			final byte[] bytes = coder.digest();
		...
		try {
			if (game.getClientState() != 10) {
				...
				if (inventory.getCount(runeEssence) != 28) {
					final RSObject Booth = objects.getNearest(bankBoothID);
					final RSObject Chest = objects.getNearest(chestID);
					...
					if (inAltarArea()) {
						final RSObject Altar = objects.getNearest(altarID);
						if (Altar != null && isOnScreen(Altar.getModel())) {
							return Action.CRAFTRUNE;
						}
						return Action.WALKTOALTAR;
					} else {
						final RSObject Ruins = objects.getNearest(ruinsID);
	...
	private int getNearestTile(final ArrayList<RSTile> path) {
		int shortest = 0;
	...
	public Point getPointOnScreen(final RSModel m, final boolean first) {
	...
	private boolean isOnScreen(final RSModel m) {
	...
	public int loop() {
		try {
			...
			if (GUI.restMode) {
				if (Rune == 556 || Rune == 557 || Rune == 558) { // Resting at a
					...
					if (walking.getEnergy() < Methods.random(29, 38)
							...
							&& walkingTowardsMusician(Musician)) {
			...
			if (Rune == 557) { // Earth runecrafting failsafes
				if (game.getPlane() == 1) {
					final RSObject Stairs = objects.getNearest(stairsID);
					for (int i = 0; i < 150; i++) {
						if (Stairs != null) {
		...
		return randomGauss(Methods.random(189, 191), Methods.random(428, 431), 251, 40);
	}
	...
	private boolean mouseOnObjectModel(final Point p, final RSObject object) {
		if (object == null) {
			return false;
		}
		final Polygon[] triangles = object.getModel().getTriangles();
	...
	private void moveMouse(final int time, final Point p) {
	...
	public void onFinish() {
		...
		final long gainedXP = skills.getCurrentExp(Skills.RUNECRAFTING)
				- startExp;
		final long gainedLevels = skills.getRealLevel(Skills.RUNECRAFTING)
				- startLevel;
	...
	public void onRepaint(final Graphics g) {
		if (game.isLoggedIn() && !game.isLoginScreen() && !stoppingScript) {
			...
			if (tab == 1) { // General
				...
				if (runesCrafted > 0) {
					final int runeRatio = runesCrafted / essenceUsed;
	...
	private int randomGauss(final int lo, final int hi, final double d,
			final double sd) {
		int rand;
		do {
	...
	private Line[] reverse(final Line[] lines) {
		final Line[] rev = new Line[lines.length];
	...
	private boolean sleepToMove(final int timeout) {
	 ...
	 */
	private boolean sleepWhile(final Condition condition,
			final RSObject targetObject, final int tileBuffer,
			final int Threshold) {
		final int Irritations = Threshold / 25;
	...
	private boolean walkingTowardsMusician(final RSNPC npc) {
		int Delta = 0;
		final int test1 = calc.distanceTo(npc); // 7
		while (Delta == 0 && players.getMyPlayer().isMoving()) {
			final int test2 = calc.distanceTo(npc); // 5
	...
	private boolean walkTile(final RSTile t, final int x, final int y) {
		final RSTile dest = new RSTile(t.getX() + random(0, x), t.getY()
				+ random(0, y));
