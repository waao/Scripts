...
public class KubkoWhiteBerries extends Script implements PaintListener,
	...
	RSArea teleGrabTileArea = new RSArea(new RSTile(3214, 3790), new RSTile(
			3230, 3813));
	...
	RSArea redDragonIsle = new RSArea(new RSTile(3189, 3821), new RSTile(3205,
			3843));
	RSArea teleGrabPlace = new RSArea(new RSTile(3211, 3790), new RSTile(3230,
			3810));
	...
	RSArea nearDitch = new RSArea(new RSTile(3127, 3498),
			new RSTile(3139, 3520));
	...
	RSTile[] toLumb2 = { new RSTile(3219, 3219),
			new RSTile(3219, 3219), new RSTile(3215, 3215),
			new RSTile(3219, 3219), new RSTile(3215, 3215),
			new RSTile(3213, 3211), new RSTile(3219, 3219),
	...
	RSArea atBerries = new RSArea(new RSTile(3207, 3808),
			new RSTile(3221, 3820));
	RSArea afterShortcut = new RSArea(new RSTile(3135, 3516), new RSTile(3138,
			3516));
	...
	RSArea varrockArea = new RSArea(new RSTile(3160, 3420), new RSTile(3232,
			3476));
	...
	RSArea lumbBank = new RSArea(new RSTile(3205, 3209),
			new RSTile(3212, 3225), 2);

	RSArea lumb2ndFloor = new RSArea(new RSTile(3205, 3209), new RSTile(3210,
			3223), 1);

	RSArea lumbridgeArea = new RSArea(new RSTile(3203, 3202), new RSTile(3226,
			3228), 0);
	...
	public int loop() {
		if (game.isLoggedIn()) {
			...
			case MAKE_CANOE:
				canoeWalking();
				sleep(3000, 4000);
				RSObject station2 = objects.getNearest(12166);
				...
				RSComponent travel = interfaces.get(53).getComponent(45);
				if (interfaces.get(53).getComponent(45).isValid()) {
			...
			case WALK_TO_LUMB_BANK:
				...
				if(game.getPlane() == 1){
					RSObject stairs2 = objects.getNearest(36774);
...
if(firstTeleGrab == true){
	...
	firstTeleGrab = false;
				...
				if (getMyPlayer().getAnimation() == -1) {
					if (!magic.isSpellSelected()) {
						RSGroundItem berryGround = groundItems.getNearest(239);
						...
						RSGroundItem berryGround2 = groundItems.getNearest(239);
			...
			case GO_TO_LAIR:
				RSObject entrance = objects.getNearest(38815);
				...
				if (needsToBank) {
					...
					if (bank.isOpen()) {
						...
						if (inventory.contains(554) && inventory.contains(563)
							...
							needsToBank = false;
							isDead = false;
							RSItem airstaff = inventory.getItem(1381);
							sleep(600, 900);
							if (lumbBanked == false && inventory.contains(1381)) {
								airstaff.doAction("Wield");
								airstaff.doAction("Wield");
								airstaff.doAction("Wield");
								RSItem wielding = (equipment
										.getItem(Equipment.WEAPON));
								...
								RSItem airstaff2 = inventory.getItem(1381);
								game.openTab(Game.TAB_EQUIPMENT);
								sleep(1000, 1500);
								RSItem wielding4 = (equipment
										.getItem(Equipment.WEAPON));
								log("we are wielding " + wielding4);
								if (equipment.getItem(Equipment.WEAPON) == airstaff2) {
									RSItem wielding2 = (equipment
											.getItem(Equipment.WEAPON));
									log("we are wielding " + wielding2);
									lumbBanked = true;
								} else {
									RSItem wielding3 = (equipment
											.getItem(Equipment.WEAPON));
			...
			case WALK_TO_SHORTCUT:
				...
				sleep(700, 1200);
				RSObject shortcut = objects.getNearest(geShortcut);
	...
	public void onRepaint(Graphics g1) {
		...
		seconds = millis / 1000;
		moneyPerHour = (int) ((berryPrice * berriesPicked) * 3600000D / (System
	...
	boolean isOpen() {
		RSObject closed = objects.getNearest(closedDeepGate);
		RSObject open = objects.getNearest(openDeepGate);
		RSObject closed2 = objects.getNearest(closedDeepGate);
		RSObject open2 = objects.getNearest(openDeepGate);
	...
	void canoeSelecting() {
		...
		if (interfaces.get(52).getComponent(24).containsText("A Waka")) {
			RSComponent makeCanoe = interfaces.get(52).getComponent(24);
