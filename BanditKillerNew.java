...
public class BanditKillerNew extends Script implements PaintListener, MessageListener {
    ...
    RSTile shop0 = new RSTile(3175, 2987), shop1 = new RSTile(3175, 2988), shop2 = new RSTile(3175, 2989), shop3 = new RSTile(3176, 2985),
            shop4 = new RSTile(3176, 2986), shop5 = new RSTile(3176, 2987), shop6 = new RSTile(3177, 2985), shop7 = new RSTile(3177, 2986),
            shop8 = new RSTile(3177, 2987), shop9 = new RSTile(3178, 2986), shop = new RSTile(3176, 2987), outsideroom = new RSTile(3172, 2977),
            insideroom = new RSTile(3172, 2976), fight = new RSTile(3176, 2972);
    RSTile[] shopTiles = {shop0, shop1, shop2, shop3, shop4, shop5, shop6, shop7, shop8, shop9};
    public int foodID, notedID, temp, freespaces, specPercent, wepID, howManySpecs, potQuant;
    public int shopkeeperID = 1917, opened = 1529, closed = 1528, temp2;
    public int pray4 = 2434, pray3 = 139, pray2 = 141, pray1 = 143, notedPrayID, notedPotID, pot4 = 9739, pot3 = 9741, pot2 = 9743, pot1 = 9745;
    public boolean foodMode = false, prayMode = false, specEquip = false, specInven = false, specWepEquipped = false, waiting = false, specMode = false,
            potMode = false, startScript = false, stop = false;
    public String state = "Loading", monster = "Bandit", fightAt;
    public long startTime = 0, runTime = 0, hours = 0, minutes = 0, seconds = 0, nextAntiban = 0, timer = 0;
    public int startXPAttack = 0, startXPDefence = 0, startXPStrength = 0, startXPHP = 0, startXPRange = 0, startXPMagic = 0, levelsGained = 0;
    ...
    public boolean needMorePots() {
        if (potMode) {
            if (inventory.getCount(pot1) < 1 && inventory.getCount(pot2) < 1 && inventory.getCount(pot3) < 1 && inventory.getCount(pot4) < 1) {
    ...
    public void spec() {
        if (specEquip) {
            ...
            if (!specWepEquipped) {
                game.openTab(4, true);
                RSItem specwep = inventory.getItem(wepID);
    ...
    public boolean needPrayPot() {
        if (((Integer.parseInt(interfaces.get(749).getComponent(4).getText()) * 100) / (skills.getRealLevel(5))) < 60) {
            return true;
        }
        return false;
    }
    ...
    public void DrinkPray() {
        if (needPrayPot() == true) {
            if (inventory.getItem(pray1) != null) {
                RSItem praypot = inventory.getItem(pray1);
                praypot.doClick(true);
                sleep(800);
                return;
            }
            if (inventory.getItem(pray2) != null) {
                RSItem praypot = inventory.getItem(pray2);
                praypot.doClick(true);
                sleep(800);
                return;
            }
            if (inventory.getItem(pray3) != null) {
                RSItem praypot = inventory.getItem(pray3);
                praypot.doClick(true);
                sleep(800);
                return;
            }
            if (inventory.getItem(pray4) != null) {
                RSItem praypot = inventory.getItem(pray4);
                praypot.doClick(true);
                sleep(800);
                return;
            }
    ...
    public boolean needToPot() {
        if (skills.getCurrentLevel(Skills.ATTACK) == skills.getRealLevel(Skills.ATTACK) || skills.getCurrentLevel(Skills.STRENGTH) == skills.getRealLevel(Skills.STRENGTH)) {
            return true;
        }
        return false;
    }
        ...
        if (potMode) {
            if (needToPot()) {
                if (inventory.getItem(pot1) != null) {
                    RSItem praypot = inventory.getItem(pot1);
                    praypot.doClick(true);
                    sleep(800);
                    return;
                }
                if (inventory.getItem(pot2) != null) {
                    RSItem praypot = inventory.getItem(pot2);
                    praypot.doClick(true);
                    sleep(800);
                    return;
                }
                if (inventory.getItem(pot3) != null) {
                    RSItem praypot = inventory.getItem(pot3);
                    praypot.doClick(true);
                    sleep(800);
                    return;
                }
                if (inventory.getItem(pot4) != null) {
                    RSItem praypot = inventory.getItem(pot4);
                    praypot.doClick(true);
                    sleep(800);
                    return;
                }
    ...
    public void dropVials() {
        for (RSItem vial : inventory.getItems(229)) {
            vial.doAction("Drop");
        }
    }
        ...
        if (foodMode) {
            if (!needEat()) {
                if (underAttack()) {
                    ...
                    RSNPC target = npcs.getNearest(monsterFilter);
                    if (target != null) {
                        if (target.isOnScreen() == true) {
                            target.doAction("Attack " + target.getName());
                            sleep(1000);
                        } else {
                            RSPath zomg4 = walking.getPath(target.getLocation());
                            zomg4.traverse();
                        }
                    }
                }
    ...
    private final Filter<RSNPC> monsterFilter = new Filter<RSNPC>() {
        ...
        public boolean accept(RSNPC t) {
            if (correctMonster(t) && t.getHPPercent() != 0 && t.isValid() && t.getAnimation() != 836 && insideSouthHouse(t)) {
                return true;
            } else {
                return false;
            }
    ...
    public boolean isCurtainOpen() {
        if (objects.getNearest(opened) != null && calc.distanceTo(objects.getNearest(opened)) < 3) {
            return true;
        }
        return false;
    }

    private boolean correctMonster(RSNPC t) {
    ...
    public int getItemStacksize(int id) {
        if (store.isOpen()) {
            RSComponent[] items = interfaces.get(620).getComponent(25).getComponents();
            for (RSComponent i : items) {
    ...
    public void openShop() {
        RSNPC shopkeeper = npcs.getNearest(shopkeeperID);
    ...
    public boolean atShop() {
        RSArea area1 = new RSArea(shopTiles);
    ...
    public void walkToRoom() {
        while (calc.distanceTo(outsideroom) > 3) {
            ...
            RSPath path1 = walking.getPath(outsideroom);
            path1.traverse();
            sleep(1500);
        }
    }
    ...
    public void walkToDoor() {
        while (calc.distanceTo(insideroom) > 3) {
            state = "Going to Shop!";
            RSPath path1 = walking.getPath(insideroom);
            path1.traverse();
            sleep(1500);
        }
    }
    ...
    public void walkToShop() {
        while (calc.distanceTo(shop) > 2) {
            state = "Going to Shop!";
            RSPath path1 = walking.getPath(shop);
            path1.traverse();
            sleep(1500);
        }
    }
    ...
    public void walkToFight() {
        while (calc.distanceTo(fight) > 2) {
            state = "Going to South Room!";
            RSPath path1 = walking.getPath(fight);
            path1.traverse();
            sleep(1500);
        }
    }
    ...
    public boolean insideSouthHouse() {
        RSArea area1 = new RSArea(new RSTile(3172, 2970), new RSTile(3176, 2972));
        RSArea area2 = new RSArea(new RSTile(3169, 2973), new RSTile(3176, 2976));
        if (area1.contains(getMyPlayer().getLocation()) || area2.contains(getMyPlayer().getLocation())) {
            return true;
        }
        return false;
    }

    public boolean insideSouthHouse(RSNPC i) {
        RSArea area1 = new RSArea(new RSTile(3172, 2970), new RSTile(3176, 2972));
        RSArea area2 = new RSArea(new RSTile(3169, 2973), new RSTile(3176, 2976));
    ...
    public void onRepaint(Graphics g1) {
        ...
        int XPChangeDefence = skills.getCurrentExp(1) - startXPDefence;
        int defencePerHour = 0;
        int XPChangeMagic = skills.getCurrentExp(6) - startXPMagic;
        int magicPerHour = 0;
            ...
            if (insideSouthHouse()) {
                if (needMoreFood() || needMorePray() || needMorePots()) {
                    ...
                    if (isCurtainOpen()) {
                        ...
                        RSObject closedDoor = objects.getNearest(closed);
    ...
    public class GUI extends javax.swing.JFrame {
        ...
        private void initComponents() {
            ...
            location.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Choose", "South Room"}));
            ...
            layout.setHorizontalGroup(
                    layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addComponent(jTabbedPane1, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 413, Short.MAX_VALUE));
            layout.setVerticalGroup(
                    layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addComponent(jTabbedPane1, javax.swing.GroupLayout.DEFAULT_SIZE, 300, Short.MAX_VALUE));
