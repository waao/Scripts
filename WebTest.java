...
public class WebTest extends Script {
	...
	public int loop() {
		if (web == null) {
			...
			web.traverse();
			Methods.sleep(50);// So it doesn't burn out your CPU.
		}
		if (web.atDestination()) {
			log(C,
					"I have reached my destination thanks for flying Aut0 Timelines!");
		}
	...
	public boolean onStart() {
		...
		final String[] bla = a.split(",");
		if (bla.length != 2) {
			return false;
		}
		tile = new RSTile(Integer.parseInt(bla[0].trim()),
				Integer.parseInt(bla[1].trim()));
		return true;
	}
