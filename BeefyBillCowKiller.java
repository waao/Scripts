				...
				for (int x = 0; x < lootpile.length; x++) {
					...
					for(int y = 0;y < loots.length; y++) {
						if(item == loots[y] && !inventory.isFull()) { 
							tiles.interact(lootLocation, "Take " + lootpile[x].getItem().getName());
//							takeItem(lootpile[x]);
							waitPlayerMoving();
							sleep(650, 800);
						}
	...
	private void climbStairs(String climb) {
		...
		if (game.getPlane() != z && objects.getNearest(lumbyStairs) != null) {
			if(!objects.getNearest(lumbyStairs).interact(climb)) climbStairs(climb);
			sleep(750, 1500);
		}
		waitPlayerMoving();
		return;
	...
	private void randomBump(RSTile loc) {
		...
		if(newTile.equals(loc) || objects.getTopAt(newTile) != null ) randomBump(players.getMyPlayer().getLocation());
		xpos = newTile.getX();
		ypos = newTile.getY();
	...
	private void openGate() {
		...
		try {
			while (objects.getNearest(gate) != null
				...
				objects.getNearest(gate).interact("Open");
				waitPlayerMoving();
				waitPlayerActing();
				sleep(500, 750);
			}
	...
	private void doCook() {
		...
		if(inventory.getSelectedItem() == null) inventory.getItem(rawMeat).interact("Use Raw beef");
		sleep(1500,1750);
		if(inventory.getSelectedItem() == null) doCook();
		if(tiles.interact(fire.getLocation(), "Raw"))	
			sleep(2500);
