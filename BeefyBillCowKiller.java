...
@ScriptManifest(authors = { "Mr. Byte" }, 
				name = "Beefy Bill Cow Killer", 
				keywords = { "Combat" }, 
				description = "Kills Cows, Loots Hides," +
...
public class BeefyBillCowKiller extends Script implements PaintListener,
	...
	private static final RSTile[] lumBank2BillPen = { new RSTile(3205, 3209), new RSTile(3208, 3209),
													  new RSTile(3213, 3209), new RSTile(3215, 3214),
													  new RSTile(3217, 3218), new RSTile(3223, 3219),
													  new RSTile(3228, 3219), new RSTile(3236, 3219),
													  new RSTile(3236, 3226), new RSTile(3231, 3230),
													  new RSTile(3226, 3235), new RSTile(3223, 3242),
													  new RSTile(3220, 3248), new RSTile(3217, 3252),
													  new RSTile(3213, 3265), new RSTile(3215, 3273),
													  new RSTile(3213, 3277), new RSTile(3205, 3280),
													  new RSTile(3196, 3280), new RSTile(3192, 3289),
													  new RSTile(3189, 3293), new RSTile(3189, 3299),
													  new RSTile(3186, 3306), new RSTile(3178, 3310),
													  new RSTile(3177, 3315)},
	...
	private static final String[] statusNames$ = { "Starting Up", "Cooking", "Eating", 
												  "Banking", "Fighting", "Looting", 
												  "Attacking", "Resting", "Burying",
												  "Dumping Junk", "Returning to Pen", "Looking for a cow",
	...
	private static final int[] hatchets = { 1349, 1351, 1353, 1355, 1357, 1359,
			1361, 6739 };
	private static final int [] arrows = {877, 878, 882, 883, 884, 885, 886, 887, 888, 889, 890, 891, 892, 893, 9706};

	/* It appears that the poisoned variety of arrow is 1 greater than the normal one.
	 * Bronze 882 * Iron 884 * Steel 886 * Mithril 888 * Adamant 890 * Rune 892
	 * Bronze Bolts: 877 
	 * Training Arrows: 9706
	 */

	private static final int [] bows = {829, 837, 839, 841, 843, 845, 847, 849, 851, 853, 9705};
	...
	private int[] junk = { 2146, 2146, 2146, 2146, 1511, 1521, 19830, 20114, 20111 };
	
	/*
	 * {conditional meat, conditional bones, conditional air staff, burnt meat, logs, sling, candles, cake} The 2146's
	 * at the beginning are place holders and harmless if never changed. The candles/cake are from the 10th anniv. stuff.
	 */
	...
	private int status = 0, meatHeld, cookedMeatHeld, rawMeatHeld, meatSpace, meatsLooted, meatCycles,
			meatsBanked, meatPrice, meatsValue;
	private int hidePrice, hidesValue, hidesBanked, hidesLooted, hidesHeld, bankCycles, hideCost = 0;
	...
	private boolean guiExit = false, runButtonPushed, iCanHazCheezBurger, iCanCookCheezBurger, update, quitNow;
	private String bone$ = "Getting Bones: Yes";
	private String meat$ = "Getting Meats: Yes";
	private String hide$ = "Getting Hides: NO!!";
	private double version = BeefyBillCowKiller.class.getAnnotation(ScriptManifest.class).version();
	...
	public File PRICE_FILE = new File(new File(
			Configuration.Paths.getScriptCacheDirectory()), "GEPrices.txt");
	...
	private action action() {
		meatHeld = (inventory.getCount(cookedMeat) + inventory
				.getCount(rawMeat));
		...
		if (billPen.contains(me)) {
			...
			} else if ((bankingHides && inventory.getCount(cowhideID) >= hidesToBank)
	...
	public boolean onStart() {
		...
		// Hides??
		log("You have "
				+ (inventory.getCount() - inventory.getCount(cowhideID) - inventory
						.getCount(bones)) + " misc items, " + countCheezBurgers() + " of that is food, " +
		...
		if(iCanCookCheezBurger) {
			int foodcount = countCheezBurgers();
			meatSpace = (28 - (inventory.getCount()
					- inventory.getCount(cowhideID) - inventory.getCount(bones)
					- inventory.getCount(rawMeat) - foodcount) - hidesToBank);
			if(meatSpace > 4)
				meatSpace = 4;
		} // limit it to 4 meats.
...
//			meatPrice = getGuidePrice(rawMeat);	
	...
	public int loop() {
		...
		if(inventory.contains(arrowID)) inventory.getItem(arrowID).doClick(true);
		
		boolean havebones = inventory.contains(bones);
		cookedMeatHeld = inventory.getCount(cookedMeat);
		rawMeatHeld = inventory.getCount(rawMeat);
		...
		if(!outOfPen && !billPen.contains(me)) returnToPen();

		if(outOfPen && gateArea.contains(me)) openGate();
		
		switch (action()) {
		case COOKING:
			RSObject fire = objects.getNearest(litFire);
			if(fire == null) {
				bePrometheus();
			} else {
				RSTile where = fire.getLocation();
		...
		case BANKING:
			...
			if (bill == null) {
				if (verbose)
					log("Bill not found?");
				walking.walkTileMM(walking.getClosestTileOnMap(billPen
						.getCentralTile()));
			...
			RSTile newTile = bill.getLocation();
			if (!bill.isOnScreen()) {
				if (verbose)
					log("Walking to Bill");
		...
		case LOOTING:
			...
			if (havebones && gettingBones && (inventory.isFull() || random(1,100)<30)) {
				status = buryBones(status);
			}
			try {
				RSTile lootLocation = loot.getLocation();
				RSGroundItem lootpile[] = groundItems.getAllAt(lootLocation);
				...
				for (int x = 0; x < lootpile.length; x++) {
					int item = lootpile[x].getItem().getID();
					for(int y = 0;y < loots.length; y++) {
						if(item == loots[y] && !inventory.isFull()) { 
//							tiles.doAction(lootpile[x].getLocation(), "Take " + lootpile[x].getItem().getName());
							takeItem(lootpile[x]);
							waitPlayerMoving();
							sleep(650, 800);
						}
				...
				if(verbose) log.log(Level.SEVERE, "Looting: ", e);
				return 5;
				}
		...
		case FIGHTING:
			try {
				if(players.getMyPlayer().getInteracting() != null
					...
					while((cow.getHPPercent() > 0 && players.getMyPlayer()
							.getInteracting() != null)) {
						if(combat.getLifePoints() < skills.getRealLevel(3) * 10 * whenToEat) eatFood(status);
						if(status == 12 || quitNow) return -1;
					}
					sleep(3000, 4000);
				}
		...
		case WALKING:
			...
			if(calc.distanceTo(lumStairs.getCentralTile()) < 3 && inventory.contains(airStaff)) 
	...
	private void climbStairs(String climb) {
		int z = 0;
		if (climb.contains("up"))
			z = 2;
		if (game.getPlane() != z && objects.getNearest(lumbyStairs) != null) {
			if(!objects.getNearest(lumbyStairs).doAction(climb)) climbStairs(climb);
			sleep(750, 1500);
		}
		waitPlayerMoving();
		return;
	...
	private void randomWalk(int badCount) {
		RSTile[] tilesArray = billPen.getTileArray();
		me = players.getMyPlayer().getLocation();
		RSTile newTile = (tilesArray[random(0, tilesArray.length -1)]);
		...
		int xpos = newTile.getX(), ypos = newTile.getY();
	...
	private void randomBump(RSTile loc) {
		int xpos = loc.getX(), ypos = loc.getY();
		RSArea bumpZone = new RSArea(xpos - 1, ypos +1, xpos + 1, ypos -1);
		RSTile[] tilesArray = bumpZone.getTileArray();
		RSTile newTile = (tilesArray[random(0, tilesArray.length)]);
		if(newTile.equals(loc) || objects.getTopAt(newTile) != null ) randomBump(me);
		xpos = newTile.getX();
		ypos = newTile.getY();
		if(verbose) log("Randomly bumped to: " + xpos + ", " + ypos + ".");
    ...
    private boolean walkPath(RSPath p) {
    	walking.setRun(true);
    	int end = calc.distanceTo(p.getEnd());
    	while(calc.distanceTo(p.getEnd()) < (end + end/2) &&calc.distanceTo(p.getEnd()) > 3){
    		p.traverse();
    		if(walking.getDestination() != null){
	...
	private void bankAtBill(int itemID, String name) {
	...
	private boolean toNPC(RSNPC npc) {
		if (!npc.isOnScreen())camera.turnTo(npc);
			
			if (!npc.isOnScreen()) {
				walking.walkTileMM(walking.getClosestTileOnMap(npc
						.getLocation()));
	...
	public RSNPC newNPC() {
		...
		RSNPC[] monsters = npcs.getAll(new Filter<RSNPC>() {
...
*/
		...
		for (int i = 0; i < monsters.length; i++) {
			int distance = calc.distanceTo(monsters[i].getLocation());
			if (distance < Dist) {
				Dist = distance;
				closest = monsters[i];
			}
	...
	private boolean lootCheck() {
		...
		int cowcount = 0, count = items.length, piles = 0, y = 0;
...
*/		
		...
		if(verbose){
			...
			log("Cows = " + cowcount);
			String z$ = "Looting...";
			if(piles < cowcount) z$ = "Attacking...";
			log(z$);
		}
        return (piles >= cowcount);
	}
	
	private int getArrows(int oldstatus) {
		status = 13;
		RSGroundItem item = groundItems.getNearest(arrowID);
		String name = item.getItem().getName();
		tiles.doAction(item.getLocation(), "Take " + name);
		sleep(400,600);
		if(groundItems.getNearest(arrowID) != null) getArrows(arrowID);
		return oldstatus;
	}
	...
	private RSGroundItem findLoot() {
		try {
			...
			if(needMeat) loot = groundItems.getNearest(rawMeat);
			if(loot == null || needMeat) return loot;

			if(gettingBones && bankingMeats && bankingHides && billPen.contains(loot.getLocation()) && loot.isOnScreen()) {
				return loot;
			}
	...
	private int eatFood(int oldstatus) {
		if(!iCanHazCheezBurger)
			return oldstatus; // We're not eating...
		status = 2;
		RSItem food = iHazCheezburgers();
		if(food != null) {
			food.doAction("Eat ");
			return oldstatus;
		} else {
	...
	public int buryBones(int oldstatus) {
		status = 8;
		if(verbose)
			log("Burying bones...");
	...
	public int dropJunk(int oldstatus, int... items) {
		status = 9;
...
/*	public void turnTo(int degrees, RSNPC n) {
	...
	private void bePrometheus() {
		...
		if(!inventory.containsOneOf(logs)) {
			...
			if(!tree.isOnScreen()) {
				walking.walkTileMM(walking.getClosestTileOnMap(tree
						.getLocation()),3,3);
			} else {
			...
			if (getMyPlayer().getAnimation() == -1) {
				int logcount = inventory.getCount(logs);
				...
				if (inventory.getCount(logs) == logcount) {
					missclicks++;
					if(verbose) log("Missed: " + missclicks);
			...
			int fc = skills.getCurrentExp(Skills.FIREMAKING);
			inventory.getItem(tinderbox).doClick(true);
			sleep(1000, 2000);
			if(inventory.containsOneOf(logs)) inventory.getItem(logs).doClick(true);
			int timer = 0;
			while(fc == skills.getCurrentExp(Skills.FIREMAKING) && timer < 20 )  // 10 sec to build a fire...
				sleep(500);
			timer++;
			if(verbose)
	...
	private void doCook() {
		if(verbose) log("doCook()");
		cookedMeatHeld = inventory.getCount(cookedMeat);
		rawMeatHeld = inventory.getCount(rawMeat);
		meatHeld = cookedMeatHeld + rawMeatHeld;
		RSObject fire = objects.getNearest(litFire);
		if(fire==null) return;
		RSTile fireTile = walking.getClosestTileOnMap(fire.getLocation());
		cookingIface = interfaces.getComponent(905, 14);
		int toCook = meatSpace - cookedMeatHeld;
		...
		if(inventory.getSelectedItem() == null) inventory.getItem(rawMeat).doAction("Use Raw beef");
		sleep(1500,1750);
		if(inventory.getSelectedItem() == null) doCook();
	...
	private boolean adjustCookLevel(int toCook) {
		...
		sleep(250);
		int count = 0;
		while(Integer.parseInt("0" + interfaces.getComponent(916, 17).getText()) != toCook) {
			if(Integer.parseInt("0" + interfaces.getComponent(916, 17).getText()) < toCook) {
				amountIncreaseButton.doClick(true);
			} else amountDecreaseButton.doClick(true);
			sleep(250,300);
			count++;
	...
	public boolean updater() {
		...
		String scriptName = BeefyBillCowKiller.class.getAnnotation(
		...
		String localJavaName = Configuration.Paths
				.getScriptsSourcesDirectory() + File.separator + javaName;
		String localJarName = Configuration.Paths
				.getScriptsPrecompiledDirectory() + File.separator + jarName;
		double revision = -1.0;
...
/* Download method from RSBot UpdateUtil by TheShadow
 * Copied to here because it wouldn't let me call it from here...some static thing (shrug)
 */
    ...
    private boolean readPrices() {
    	try {
			...
			while ((line = in.readLine()) != null) {
				if (line.contains(":")) {
					opts = line.split(":");
					if(!opts[0].equals(getDate()) || opts[2] =="Cowhide" || opts[2] == "Raw Meat" ) {
	...
	public void onRepaint(Graphics render) {
		...
		g.drawLine((int) m.getX() - 3, (int) m.getY() + 3, (int) m.getX() + 3,
				(int) m.getY() - 3);
		g.drawLine((int) m.getX() - 3, (int) m.getY() - 3, (int) m.getX() + 3,
				(int) m.getY() + 3);
		...
		if(showPaint) {
			...
			hph = hidesLooted * 3600000D / (System.currentTimeMillis() - startTime);
			mph = meatsLooted * 3600000D / (System.currentTimeMillis() - startTime);
			...
			g.drawString("Beefy Bill Cow Killer Run Time: " + format(runTime, true)
					+ " v" + version + " Mr. Byte", x,
					(y - (spcng * 3)));
			if(bankingHides || bankingMeats) {
				if (bankingHides && !bankingMeats) {
					...
					g.drawString("Hides Looted/hr: " + whole.format(hph)
							+ "  Gold Per Hour: " + whole.format(gph), x, y + (spcng*7));
				} else if(bankingMeats && bankingHides) {
					g.drawString("Hides Banked: " + hidesBanked
							+ "   Hides Looted: " + hidesLooted
							+ "   Meats Banked: " + meatsBanked
							+ "   Meats Looted: " + meatsLooted
							+ "   Market Value: " + (hidesValue + meatsValue), x, y-spcng);
					g.drawString("Hides Looted/hr: " + whole.format(hph)
	...
	public class BeefyGUI extends JFrame {
		...
		private void runButtonActionPerformed(ActionEvent e) {
			...
			if(iCanHazCheezBurger) {
				whenToEat = (double) HPPercentToEat.getValue()/100; // Convert 75 to .75
			...
			if(bankingHides) loots[1] = cowhideID;
			if(gettingBones) loots[2] = bones;
			if(bankingMeats) loots[0] = rawMeat;
			if(bankingStaffs) 
				loots[4] = airStaff;
			else 
				junk[2] = airStaff;
		...
		private void iCanHazCheezBurgerActionPerformed(ActionEvent e) {
		...
		private void getHidesItemStateChanged(ItemEvent e) {
			hideBankCount.setEnabled((guiBankHides.isSelected() && !guiBankMeat
					.isSelected()));
		...
		private void bankingMeatItemStateChanged(ItemEvent e) {
			hideBankCount.setEnabled((guiBankHides.isSelected() && !guiBankMeat
					.isSelected()));
			howManyLabel.setEnabled(hideBankCount.isEnabled());
			guiBuryBones.setEnabled(guiBankHides.isSelected()
			...
			contentPane.setLayout(new BorderLayout());
			{
				...
				dialogPane.setPreferredSize(new java.awt.Dimension(533, 450));
				{
					...
					dialogPane.add(contentPanel, BorderLayout.CENTER);
					{
						...
						guiBuryBones.setFont(new Font("Arial", Font.PLAIN,
								12));
						guiBuryBones
						.setToolTipText("The much asked for option to turn off bone grabbing. Un-check this to not grab bones.");
					...
					}
					{
						...
						guiBankHides.setFont(new Font("Arial", Font.PLAIN,
								12));
					...
					}
					{
						...
						guiBankMeat.setFont(new Font("Arial", Font.PLAIN,
								12));
					...
					}
					{
						...
						guiPickStaff.setFont(new Font("Arial", Font.PLAIN,
								12));
						guiPickStaff
						.setToolTipText("As requested by HawkenX");
					...
					}
					{
						...
						restWhenTired
						.setText("Rest when out of run energy");
						restWhenTired.setSelected(true);
						restWhenTired.setFont(new Font("Arial", Font.PLAIN,
								12));
						restWhenTired
						.setToolTipText("You'll be running alot. Rest when you're tired?");
...
/*					{
						getFoodWarning = new JLabel();
						contentPanel.add(getFoodWarning);
						getFoodWarning
						.setText("<html><center>* You need a hatchet and tinderbox to enable this function</center></html>");
						getFoodWarning.setFont(new Font("Arial",
								Font.ITALIC, 10));
						getFoodWarning.setEnabled(true);
						getFoodWarning
...
*/
					{
						...
						guiGetFood.setEnabled((guiEatFood.isSelected() && (inventory
								.contains(tinderbox) && inventory
								.containsOneOf(hatchets))));
						
						guiCheckForUpdates = new JCheckBox();
						contentPanel.add(guiCheckForUpdates);
						guiCheckForUpdates.setText("Download Updates?");
						guiCheckForUpdates
						.setToolTipText("Enable this to download updates from Mr. Byte.");
						guiCheckForUpdates.setFont(new Font("Arial",
								Font.PLAIN, 12));
						...
						beVerbose
						.setToolTipText("Enable extra Log Messages, AKA: Log SPAM!");
						beVerbose
						.setFont(new Font("Arial", Font.PLAIN, 12));
					...
					}
					{
                        ...
                        HPPercentToEat.setBorder(new TitledBorder("Eat at "
                                        + HPPercentToEat.getValue() + "% HP"));
						HPPercentToEat.setFont(new Font("Arial",
								Font.PLAIN, 12));
						HPPercentToEat
					...
					}
					{
						...
						hideBankCount.setModel(new DefaultComboBoxModel(
								new String[] { "10 Hides", "20 Hides" }));
...
//		private JLabel getFoodWarning;
	...
	public void messageReceived(MessageEvent e) {
		...
		if (e.getID() == MessageEvent.MESSAGE_SERVER) {
			...
			if(verbose) log(message + " " + e.getID());
...
/* Changelog:
 * 
 * v3.25 Changelog started.
 *    Changed GE price storage method to use item ID's rather than names
 *    Eliminated the skills[i] array, as it was not really needed.
 *    Cleaned up sleep(random(x,x)) sleep(x,x)
 *    Changed update checker to now look on server at script file and extract 
 *    the posted version from it instead of reading a version file.
 *    
 * v3.26
 *    Added safety-checks to the ID field in the price file, due to errors if reading old price file with latest code.
 *    
 * v3.27
 *    Improved some visuals on the updater GUI, changed the restart message for jar's to reflect the fact
 *    that you need to restart the bot for the new .jar to appear.
 *    Removed compile code, as it's not working locally...
 *    
 * v3.28
 * 	  Fixed issue in randomWalk() causing script to hang and bot to idle and timeout.
 * 	  Improved portability of update method.   
 *    
 * v3.29
 *    Added "Get Air Staffs" option.
 *    
 * v3.30
 *    Moved price file location to comply with new security stuff.
 *    
 * v3.31
 *    Changed "GlobalConfiguration" to "Configuration" to match bot change.
