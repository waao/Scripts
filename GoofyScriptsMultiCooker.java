...
public class GoofyScriptsMultiCooker extends Script  implements PaintListener, MessageListener, MouseListener{
	...
	if(objects.getNearest(25730) != null || npcs.getNearest(2271) != null){
	if(fishSelected == true) {

		if(location == "kharid"){
			RSTile playerloc = getMyPlayer().getLocation();
			RSItem rawfood = inventory.getItem(rawID);
			...
			if (!walking.isRunEnabled() && walking.getEnergy() > random(60, 75)) {
				walking.setRun(true);
			}

			if(players.getMyPlayer().isMoving()) {
				sleep(500);
			}else if(inventory.getCount(rawID) > 0 && count > inventory.getCount(rawID)) {
				count = inventory.getCount(rawID);
				sleep(3500);  //2000 is an example if your return is 500
				mouseAntiban();
			} else {
				RSObject booth = objects.getNearest(35648);
				RSObject range = objects.getTopAt(new RSTile(3271,3181));
		...
		sleep(500);
			...
			mouseAntiban();
				...
				RSObject fire = objects.getTopAt(new RSTile(3043,4973));
				RSNPC bankguy = npcs.getNearest(2271);
					...
					bank.close();
						...
						if(bankguy != null) {
							if (calc.pointOnScreen(bankguy.getScreenLocation())) {
								...
								RSTile bankguytile = bankguy.getLocation();
								if (bankguytile != null) {
	...
	public void mouseClicked(MouseEvent e) {
		if(fishSelected == true && location != "null"){
			...
			if(showPaint == true){
				if(e.getPoint().x > 4 && e.getPoint().x < 79 && e.getPoint().y > 323 && e.getPoint().y < 337) {
					fishSelected = false;
					mouseinput();
				}
			}
			...
			//fish1
			if(e.getPoint().x > 35 && e.getPoint().x < 135 && e.getPoint().y > 373 && e.getPoint().y < 393){
			fishSelected = true;
			rawID = 317;
			}
			if(e.getPoint().x > 150 && e.getPoint().x < 250 && e.getPoint().y > 373 && e.getPoint().y < 393){
			fishSelected = true;
			rawID = 13435;
			}
			if(e.getPoint().x > 265 && e.getPoint().x < 365 && e.getPoint().y > 373 && e.getPoint().y < 393){
			fishSelected = true;
			rawID = 321;
			}
			if(e.getPoint().x > 380 && e.getPoint().x < 480 && e.getPoint().y > 373 && e.getPoint().y < 393){
			fishSelected = true;
			rawID = 327;
			}
			...
			//fish2
			if(e.getPoint().x > 35 && e.getPoint().x < 135 && e.getPoint().y > 398 && e.getPoint().y < 418){
			fishSelected = true;
			rawID = 345;
			}
			if(e.getPoint().x > 150 && e.getPoint().x < 250 && e.getPoint().y > 398 && e.getPoint().y < 418){
			fishSelected = true;
			rawID = 335;
			}
			if(e.getPoint().x > 265 && e.getPoint().x < 365 && e.getPoint().y > 398 && e.getPoint().y < 418){
			fishSelected = true;
			rawID = 331;
			}
			if(e.getPoint().x > 380 && e.getPoint().x < 480 && e.getPoint().y > 398 && e.getPoint().y < 418){
			fishSelected = true;
			rawID = 359;
			}

			//fish3
			if(e.getPoint().x > 15 && e.getPoint().x < 115 && e.getPoint().y > 423 && e.getPoint().y < 443){
			fishSelected = true;
			rawID = 377;
			}
			if(e.getPoint().x > 130 && e.getPoint().x < 230 && e.getPoint().y > 423 && e.getPoint().y < 443){
			fishSelected = true;
			rawID = 363;
			}
			if(e.getPoint().x > 245 && e.getPoint().x < 345 && e.getPoint().y > 423 && e.getPoint().y < 443){
			fishSelected = true;
			rawID = 371;
			}
			if(e.getPoint().x > 360 && e.getPoint().x < 460 && e.getPoint().y > 423 && e.getPoint().y < 443){
			fishSelected = true;
			rawID = 7944;
			}
			...
			if(e.getPoint().x > 130 && e.getPoint().x < 230 && e.getPoint().y > 448 && e.getPoint().y < 468){
			fishSelected = true;
			rawID = 383;
			}
			if(e.getPoint().x > 245 && e.getPoint().x < 345 && e.getPoint().y > 448 && e.getPoint().y < 468){
			fishSelected = true;
			rawID = 15270;
			}
			...
			if(e.getPoint().x > 275 && e.getPoint().x < 375 && e.getPoint().y > 373 && e.getPoint().y < 393){
			location = "den";
			showPaint = true;
			env.setUserInput(2);
			}
	...
	public void onFinish(){
		...
		log("You cooked "+cooked+" fish" +" and gained "+expGained +"exp");
	...
	private final Color color7 = new Color(0, 255, 255, 129);
	...
	public void onRepaint(Graphics g1) {
		...
		if(fishSelected == true && location != "null") {
			...
			g.drawString("Current Level: " + currLevel + " (+" + lvlsGained + ")", 9, 402);
			...
			if(mouse.getLocation().getX() > 35 && mouse.getLocation().getX() < 135 && mouse.getLocation().getY() > 373 && mouse.getLocation().getY() < 393) {
				g.setColor(color5);
				g.fillRect(35, 373, 100, 20);
			} else if(mouse.getLocation().getX() > 150 && mouse.getLocation().getX() < 250 && mouse.getLocation().getY() > 373 && mouse.getLocation().getY() < 393) {
				g.setColor(color5);
				g.fillRect(150, 373, 100, 20);
			} else if(mouse.getLocation().getX() > 265 && mouse.getLocation().getX() < 365 && mouse.getLocation().getY() > 373 && mouse.getLocation().getY() < 393) {
				g.setColor(color5);
				g.fillRect(265, 373, 100, 20);
			} else if(mouse.getLocation().getX() > 380 && mouse.getLocation().getX() < 480 && mouse.getLocation().getY() > 373 && mouse.getLocation().getY() < 393) {
				g.setColor(color5);
				g.fillRect(380, 373, 100, 20);

			} else if(mouse.getLocation().getX() > 35 && mouse.getLocation().getX() < 135 && mouse.getLocation().getY() > 398 && mouse.getLocation().getY() < 418) {
				g.setColor(color5);
				g.fillRect(35, 398, 100, 20);
			} else if(mouse.getLocation().getX() > 150 && mouse.getLocation().getX() < 250 && mouse.getLocation().getY() > 398 && mouse.getLocation().getY() < 418) {
				g.setColor(color5);
				g.fillRect(150, 398, 100, 20);
			} else if(mouse.getLocation().getX() > 265 && mouse.getLocation().getX() < 365 && mouse.getLocation().getY() > 398 && mouse.getLocation().getY() < 418) {
				g.setColor(color5);
				g.fillRect(265, 398, 100, 20);
			} else if(mouse.getLocation().getX() > 380 && mouse.getLocation().getX() < 480 && mouse.getLocation().getY() > 398 && mouse.getLocation().getY() < 418) {
				g.setColor(color5);
				g.fillRect(380, 398, 100, 20);

			} else if(mouse.getLocation().getX() > 35 && mouse.getLocation().getX() < 135 && mouse.getLocation().getY() > 423 && mouse.getLocation().getY() < 443) {
				g.setColor(color5);
				g.fillRect(35, 423, 100, 20);
			} else if(mouse.getLocation().getX() > 150 && mouse.getLocation().getX() < 250 && mouse.getLocation().getY() > 423 && mouse.getLocation().getY() < 443) {
				g.setColor(color5);
				g.fillRect(150, 423, 100, 20);
			} else if(mouse.getLocation().getX() > 265 && mouse.getLocation().getX() < 365 && mouse.getLocation().getY() > 423 && mouse.getLocation().getY() < 443) {
				g.setColor(color5);
				g.fillRect(265, 423, 100, 20);
			} else if(mouse.getLocation().getX() > 380 && mouse.getLocation().getX() < 480 && mouse.getLocation().getY() > 423 && mouse.getLocation().getY() < 443) {
				g.setColor(color5);
				g.fillRect(380, 423, 100, 20);

			} else if(mouse.getLocation().getX() > 150 && mouse.getLocation().getX() < 250 && mouse.getLocation().getY() > 448 && mouse.getLocation().getY() < 468) {
				g.setColor(color5);
				g.fillRect(150, 448, 100, 20);
			} else if(mouse.getLocation().getX() > 265 && mouse.getLocation().getX() < 365 && mouse.getLocation().getY() > 448 && mouse.getLocation().getY() < 468) {
				g.setColor(color5);
			...
			if(mouse.getLocation().getX() > 150 && mouse.getLocation().getX() < 250 && mouse.getLocation().getY() > 373 && mouse.getLocation().getY() < 393) {
				g.setColor(color5);
				g.fillRect(150, 373, 100, 20);
			} else if(mouse.getLocation().getX() > 275 && mouse.getLocation().getX() < 375 && mouse.getLocation().getY() > 373 && mouse.getLocation().getY() < 393) {
				g.setColor(color5);
			...
			g.setColor(color8);
			int[] X = new int[]{515, (int) mouse.getLocation().getX(), 800, 800};
			int[] Y = new int[]{0, (int) mouse.getLocation().getY(), 170, 0};
